...
import { sendReviewRequestEmail } from "@/lib/review-requests";
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: request } = await supabase
    ...
    .select("customer_name, customer_email, token, clicked_at, reminder_sent_at")
    .eq("id", id)
    .maybeSingle();
  ...
  try {
    await sendReviewRequestEmail({
      ...
      token: request.token,
      senderName: admin.full_name?.trim().split(/\s+/)[0] || "The team",
      isReminder: true,
    });
  } catch (err) {
